<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;
use App\Models\Depart;
use DB;

class DepartController extends Controller
{
    public function __construct()
    {
        $this->middleware('auth');
    }
    
   /* public function depart(){
    $data=depart::all();
    return view('depart', ['depart'=>$data]);
   }
   
   function Add_Depart(Request $req){
      $depart = new Depart;
      $depart->nameDepart=$req->nameDepart;
      $depart->postby = auth()->user()->name;
      $depart->postdate=date('Y-m-d H:i:s');
      $depart->save();
      return redirect()->back()->with('message', 'Add Data Was Succesfully');
   }

   function delete_depart($id_depart){
      $data=depart::find($id_depart);
      $data->delete();
      return redirect('depart');
     }

     function edit_depart(Request $req){
      $data=depart::find($req->id_depart);
      $data->nameDepart=$req->nameDepart;
      $data->postby = auth()->user()->name;
      $data->postdate=date('Y-m-d H:i:s');
      $data->save();
      return redirect('depart');
   } */

   public function index(Request $req)
   {
    $draw = $req->draw;
    $start = $req->start;
    $rowpage = $req->length;

    $columnIndex_arr = $req->order;
    $columnName_arr = $req->columns;
    $columnName_arr2 = json_decode($columnName_arr,true);
    $search_arr = $req->search;

    $columnOrderIndex = $columnIndex_arr[0]['column']; // Column index
    $searchValue = $search_arr['value'];

    // TOTAL RECORD
    $totalRecords = Depart::select('count(*) as allcount')->count();
    $totalRecordsFilter = Depart::select('count(*)as allcount')->where('nameDepart', 'like', '%' .$searchValue. '%')->count();

    $records =  Depart::where('depart.nameDepart', 'like', '%' .$searchValue. '%')
    ->skip($start)
    ->take($rowpage)
    ->get();

    return response()->json(['draw'=>$draw, 'recordsTotal'=> $totalRecords, 'recordsFiltered'=> $totalRecordsFilter, 'data' => $records]);

   }

   public function store(Request $req){
    if ($req->id_depart == ''){
       Depart::create([
          'nameDepart' => $req->nameDepart,
          'postby' => auth()->user()->name,
          'postdate' => date('Y-m-d H:i:s')
       ]);
       return response()->json(['success' => true,'result' => 'Insert Successfully']);
    }
    else{
       $data=depart::find($req->id_depart);
       $data->nameDepart=$req->nameDepart;
       $data->postby = auth()->user()->name;
       $data->postdate=date('Y-m-d H:i:s');
       $data->save();
       return response()->json(['success' => true,'result' => 'Update Successfully']);
    }
 }

    public function show($id_depart)
    {
       error_log($id_depart);
       $data = Depart::find($id_depart);
       return response()->json($data);
    }

    public function delete($id_depart)
    {
       $data=depart::find($id_depart);
       $data->delete();
       return response()->json(['success' => true,'result' => 'Data Already Deleted']);
    }
}

