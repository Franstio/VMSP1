...
class JotFormController extends Controller
{
    ...
    protected $client;
    protected $apiKey;
    protected $apiKeyNew = "328a3a2385ce0df6b089763ffb27d7e9";
    public function __construct()
    {
        $this->client = new Client(['headers'=>["APIKEY"=>$this->apiKeyNew]]);
        $this->apiKey = '8b51453d5fb0a64620369de994b3b0c3';
    }
    public function storePermitManual(Request $req)
    {
        $permit = $req->permit;
        $image = str_replace('data:image/png;base64,', '', $permit['sign']);
        $image = str_replace(' ', '+', $image);
        $imageName =  Str::random(10).'.'.'png';
        $db = \DateTime::createFromFormat('Y-m-d\TH:i', $permit["startDate"]);
        $permit["startDate"] = $db->format('Y-m-d H:i');
        
        $db = \DateTime::createFromFormat('Y-m-d\TH:i', $permit["endDate"]);
        $permit["endDate"] = $db->format('Y-m-d H:i:s');
        $permit["subDate"] = date('Y-m-d H:i:s');
        $permit["sign"] = $imageName;
        if ($permit["purpose"] == "WORKING")
        {
            $permit["nameLocation"] ="";
            $permit["supplyBarang"] = "";
        }
        else if ($permit["purpose"] == "SUPPLY")
        {
            $permit["desk"] = "";
            $permit["permitNo"] = "";
        }
        if ($permit["bawaBarang"] == "TIDAK")
            $permit["barangBawaan"] = "";
        $permit["status"] = "waitingadmin";
        Permit::create($permit);
        Storage::disk('public')->put("sign/".$imageName,base64_decode($image));
    }
    public function storeRegisterManual(Request $req)
    {
        $validator = $req->validate( [
            'visitor.nameVendor'=>'bail|required',
            'visitor.email' => 'bail|email|required',
            'visitor.asalVendor'=>'bail|required',
            'visitor.nik'=>'bail|required',
            'visitor.namaVisitor'=>'required',
            'visitor.gender'=>'required',
            'visitor.jabatan'=>'required',
            'visitor.sign'=>'required',
            'visitor.q1'=>'required',
            'visitor.q2'=>'required',
            'visitor.q3'=>'required',
            'visitor.q4'=>'required',
        ...
        $visitor = $req->visitor;
        $image = str_replace('data:image/png;base64,', '', $visitor['sign']);
        $image = str_replace(' ', '+', $image);
        $imageName =  Str::random(10).'.'.'png';
        $photo = str_replace('data:image/jpeg;base64,','',$visitor['photo']);
        $photo= str_replace(' ','+',$photo);
        $photoName = $visitor["nik"].".jpeg";
        $visitor["date"] = date('Y-m-d H:i:s');
        $visitor["sign"] = $imageName;
        $visitor["typeVisitor"] = "Non Residence";
...
/*            'q1' => $dictJF->q1,
    ...
    public function storeRegister()
    {
        ...
        $response = $this->client->request('GET', "https://api.jotform.com/form/222291846212452/submissions", [
            'verify' => false,
            'query' => [
                'apiKey' =>  $this->apiKey,
            ],
        ...
        foreach ($hasil['content'] as $data) {
            ...
            if ($data['status'] == 'ACTIVE') {
                ...
                $dictJF->q1 = $data['answers']['146']['answer'];

                $dictJF->q2 = $data['answers']['147']['answer'];

                $dictJF->q3 = $data['answers']['148']['answer'];

                $q3b = $data['answers']['148']['answer'] == "YA"
                ? $data['answers']['150']['answer']
                : "";

                $dictJF['q3b'] = $q3b;

                $dictJF->q4 = $data['answers']['153']['answer'];

                $dictJF->q5 = $data['answers']['154']['answer'];

                $dictJF->q6 = $data['answers']['155']['answer'];

                $dictJF->photo = $data['answers']['100']['answer'];

                $dictJF->sign = $data['answers']['156']['answer'];
    
                $base64Image = base64_encode(file_get_contents($dictJF->photo));
                $filename = $dictJF->nik.'.png';
                file_put_contents(public_path().'/storage/'.$filename, base64_decode($base64Image));
                error_log(public_path().'/storage/'.$filename);
                ...
                //$dictJF->save();
                //submissionId = $data['id'];
                $response = $this->client->request('delete', "https://api.jotform.com/form/222291846212452/submissions", [
                    'verify' => false,
                    'query' => [
                        'apiKey' =>  $this->apiKey,
                    ],
    ...
    public function storePermit(Request $req)
    {
