...
<main id="main" class="main">
  <section class="section dashboard">
    <div class="d-sm-flex align-items-center justify-content-between">
      <h4 class="mt-5 mb-2 text-gray-800"><strong>Visitor List</strong></h4>
    </div>
    ...
    <div class="row">
            ...
            <div class="modal-body">
              <div class="row">
                ...
                <div class="col-md-3">
                  ...
                  <input type="text" class="form-control" name="nomorBadge" id="nomorBadge" disabled>
                </div>
              </div>
              <div class="mb-1">
      ...
      <div class="modal" tabindex="-1" id="wrongBadge">
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-body">
              <h5 class="text-red center"><strong>Wrong badge! Please use the authorized host badge</strong></h5>
            </div>
      ...
      <div class="modal fade" id="eApproval" tabindex="-2" aria-labelledby="exampleModalCenterTitle" aria-hidden="true" role="dialog">
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
        ...
        <div class="modal-dialog modal-lg modal-dialog-scrollable modal-dialog-centered modal-sm" role="document">
          <div class="modal-content">
            ...
            <div class="modal-body">
              <div class="d-flex flex-row gap-2 justify-content-around">
                ...
                <div class="d-flex flex-column gap-2">
                    ...
                    <label>Perusahaan : <span id="confirmBadgeVendor">PANASONIC CORPORATION</span></label>
                    <label>Nama Host : <span id="confirmBadgeHost">GALIH GALUH GIRI GILBERTO</span></label>
                    <label>Lokasi : <span id="confirmBadgeLokasi">Meeting Room - Zone B</span></label>
                    <span id="confirmWithoutMedia" class="d-none text-success">without Recordable media</span>
                </div>
                <div>
                <div class="d-flex flex-row align-items-center ">
                  <img src="assets/img/id-card.png" height="80" alt=""> <span id="badge1" class="badge-number text-center text-bold">12</span>
                </div>
                </div>
              </div>
            </div>
            <div class="modal-footer">
              ...
              <span class="text-danger w-100 text-center">Press "Esc" to cancel and close this window</span>
            </div>
          </div>
        </div>
      </div>
        ...
        $(document).keydown(function(e) {
          ...
          if (e.key=='Enter'){
            if (!badge || badge==null)
            {
              ...
              if (topUser.barangBawaan && topUser.barangBawaan != null && topUser.barangBawaan != '')
              {
        ...
        function getPermitStatusByBadge(strBadge) {
          ...
          $.ajax({
            ...
            success: function(data) {
              ...
              } else if (data.typeVisitor === "Residence" && (data.purpose == "Working" || data.purpose == "WORKING")) {
                // update checkin
                updateToCheckin(data.id);
              } else {
          ...
          $.ajax({
            ...
            success: function(data) {
              ...
              for (i = 0; i < data.waitinglist.length; i++) {
                ...
                html += '        <img src="/storage/' + data.waitinglist[i].photo + ' " alt="Profile" style="padding-left: 30px;" class=" me-3" height="70">'
                html += '      </div>'
                html += '      <div class="ps-4" style="margin-left: 35px;">'
                html += '        <h8 class="text-dark" style="font-size: 20px"><strong> ' + data.waitinglist[i].namaVisitor + '</strong> </h8><br>'
                html += '        <h8 class="text-dark" style="font-size: 20px"><strong> ' + data.waitinglist[i].nik + '</strong> </h8><br>'
                html += '        <h8 class="text-dark" style="font-size: 15px"> ' + data.waitinglist[i].nameVendor + ' </h8><br>'
                html += '        <br>'
                html += '         <h8 class="text-dark" style="font-size: 15px">Host: ' + data.waitinglist[i].name.split(':')[0] + '</h8><br>';
                html += '        <h8 class="text-dark" style="font-size: 15px">To. ' + data.waitinglist[i].nameLocation + ' - Zone ' + data.waitinglist[i].zone + '</h8><br>'
                html += '        <h8 class="text-dark" style="font-size: 15px">Purpose : ' + data.waitinglist[i].purpose + ' </h8><br> '
                if (data.waitinglist[i].barangBawaan==null || data.waitinglist[i].barangBawaan == "")
                  html += '        <h8 class="text-red" style="font-size: 15px"><strong>  </strong></h8> '
                else
                {
                ...
                html += '        <h8 class="text-red" style="font-size: 15px"><strong> ' + bw[i].JenisMedia  + ' </strong></h8> '
                  }
                }
                html += '      </div>'
                // html += '      <div class="ps-5">'
                ...
                html += '      <div class=" ps-5 ">'
                html += '       <span >'
                html += '          <img src="/img/pencil-square.svg" alt="Profile" class=" me-2" onClick="EditTransaksi(' + data.waitinglist[i].id + ',' + data.waitinglist[i].noBadge + ',' + data.waitinglist[i].noVest + ')" style="border-top-style: solid;padding-bottom: 20px; " data-bs-toggle="tooltip" data-bs-placement="top" title="" aria-label="Views">'
                html += '        </span>'
                html += '        <br><br>'
                html += '      <div class="d-flex flex-column gap-2 align-items-stretch">';
                html += '      <button type="button" class="btn w-100 btn-danger" onClick=reject(' + data.waitinglist[i].id + ') data-bs-toggle="modal" data-bs-target="#ModalReject">reject </button>'
                html += '      <button type="button" class="btn btn-info" onClick=showEApproval('+ data.waitinglist[i].permitID+')>Show E-Approval</button>';
                html += '      </div>';
